...
## [2.0.0] - 2025-11-14 - ALL-IN-ONE AUTOMATION SYSTEM 🚀🎮

### Major Release - Complete Automation Orchestration

**NEW: 10-Module All-in-One System**

We've built a comprehensive automation orchestrator that manages **every aspect** of Bitburner gameplay from a single script!

**New Files**:
- ✅ `utils/auto-manager.js` - Enhanced all-in-one orchestrator (10 modules)
- ✅ `modules/faction-manager.js` - Intelligent faction automation
- ✅ `modules/augmentation-tracker.js` - Optimal aug purchase planning
- ✅ `modules/company-automator.js` - Career management automation
- ✅ `modules/bladeburner-commander.js` - Bladeburner operations
- ✅ `modules/gang-manager.js` - Gang management & ascension
- ✅ `modules/corporation-manager.js` - Corporation tracking
- ✅ `modules/go-commander.js` - Go game automation (wraps go4.js)
- ✅ `modules/README.md` - Complete module documentation
- ✅ `go4.js` - Advanced 5-stage Go AI strategy

**What It Does**:

The new `auto-manager.js` coordinates 10 automation modules in priority order:

1. **Hacking** - Target analysis & smart-batcher deployment
2. **Servers** - Automated purchasing/upgrading with ROI analysis
3. **Hacknet** - Node management and optimization
4. **Factions** - Auto-join factions & reputation farming
5. **Augmentations** - Purchase planning with price multiplier tracking
6. **Companies** - Job applications & promotions
7. **Bladeburner** - Operations, contracts, skill upgrades
8. **Gangs** - Recruitment, ascension, territory warfare
9. **Corporations** - Division tracking & recommendations
10. **Go** - Background Go game automation
...
run utils/auto-manager.js --no-factions --no-companies
...
- `--no-factions` - Disable faction automation
- `--no-companies` - Disable company automation
- `--no-augmentations` - Disable aug tracking
- `--no-bladeburner` - Disable Bladeburner
- `--no-gangs` - Disable gang management
- `--no-corporations` - Disable corp tracking
- `--no-go` - Disable Go automation
- `--no-hacknet` - Disable hacknet investing
- `--no-servers` - Disable server purchasing
- `--no-hacking` - Disable hacking operations

**Key Features**:

🎯 **Decision Coordination**
- Collects decisions from all active modules
- Prioritizes by importance (high > medium > low)
- Executes one action per cycle
- Shows pending actions for awareness
- Handles conflicts intelligently

🔓 **Graceful Degradation**
- Checks API access for each module
- Shows "locked" status when APIs unavailable
- Other modules continue working normally
- Unlocks automatically as you progress

📊 **Unified Monitoring**
- Single status display for all modules
- Shows income from all sources
- Tracks recent actions (last 5)
- Module status indicators

⚙️ **Smart Automation**
- Factions: Auto-joins best factions, farms reputation
- Companies: Applies to jobs, seeks promotions
- Augmentations: Plans optimal purchase order
- Bladeburner: Manages stamina, upgrades skills
- Gangs: Ascends at power-of-2 brackets
- Go: Plays games in background for rewards

**Module Requirements**:

| Module | Requirements |
|--------|--------------|
| Hacking/Servers/Hacknet | None (available from start) |
| Factions/Companies/Augs | SF4 (Singularity) |
| Bladeburner | SF6 or SF7 |
| Gangs | SF2 + Join gang |
| Corporations | SF3 + $150b |
| Go | Go minigame unlocked |

**Go Game Integration**:

The `go4.js` script implements an advanced 5-stage Go strategy:
1. **Base Building** - Establishes edge territory
2. **Column Creation** - Forms "eyes" for safety
3. **Expansion** - Priority-based territory growth
4. **Cleanup** - Solidifies base structure
5. **Carpet Bombing** - Fills remaining spaces

Opponents: Netburners, The Black Hand, Daedalus, Illuminati

**Example Workflow**:

Every 5 minutes, the system:
1. ✅ Analyzes hacking targets → Retargets if better option found
2. ✅ Evaluates server/hacknet investments → Purchases if ROI is good
3. ✅ Coordinates module decisions → Executes highest priority:
   - Join new faction
   - Apply to company
   ...
   - Ascend gang member
   - Start Go automation
4. ✅ Updates unified status display

**Benefits**:
- 🎮 **Truly Idle** - Manages everything while you're AFK
- 📈 **Optimal Progression** - Smart decisions based on game state
- 🔄 **Full Coverage** - Every game system automated
- ⚡ **Low Overhead** - Efficient decision-making every 5 minutes
- 🎯 **Priority System** - Critical actions happen first

**Impact**:
This is the **ultimate Bitburner automation system**. Set it and forget it - the script handles:
- Making money (hacking + hacknet + stocks)
- Joining factions & farming reputation
- Working at companies & getting promotions
- Planning augmentation purchases
- Running Bladeburner operations
- Managing your gang
- Playing Go games for rewards
- Purchasing/upgrading servers

**Version Notes**:
- Major version bump to 2.0.0 (significant new functionality)
- All existing scripts remain fully compatible
- Existing auto-manager.js users: seamless upgrade with new modules
- New modular architecture for easy extension

**Credits**: 
- Go strategy inspired by advanced Go AI techniques
- Gang ascension logic based on bitbotter's power-of-2 approach
- Community feedback on automation priorities
